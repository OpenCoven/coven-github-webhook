...
function githubFileRef(task: JsonObject, path: string): string {
  ...
  for (const detail of details) {
    const currentPath = String(detail.path || "").replaceAll("\\", "/");
    const previousPath = String(detail.previous_path || "").replaceAll("\\", "/");
    const status = String(detail.status || "").toLowerCase();
    if ((currentPath === path && status === "removed") || (previousPath === path && previousPath !== currentPath)) {
      return String(evidence.base_sha || evidence.head_sha || evidence.workspace_head_sha || task.default_branch || "").trim();
    }
  }
  return String(evidence.head_sha || evidence.workspace_head_sha || task.default_branch || "").trim();
}
...
function githubKnownFileSet(task: JsonObject, review: JsonObject = {}): Set<string> {
  ...
  const addFile = (value: JsonValue | undefined) => {
    const target = parseRepoRelativePath(String(value || ""));
    if (target) knownFiles.add(target.path);
  };
  ...
  return text
    ...
    .map((segment, index) => {
      ...
      return segment.replace(barePathPattern, (match, prefix: string, rawPath: string) => {
        if (!bareFileMentionAllowed(rawPath, knownFiles)) {
          return match;
        }
        const plain = markdownCodeSpan(rawPath);
        const linked = githubFileMarkdown(task, rawPath, null, knownFiles);
        return linked === plain ? match : `${prefix}${linked}`;
      });
    })
    .join("");
}
...
function linkInlineCodeFileMentions(task: JsonObject, text: string, knownFiles: Set<string>): string {
  ...
  while (cursor < text.length) {
    ...
    const alreadyLinkText = text[cursor - 1] === "[" && text.slice(closingEnd, closingEnd + 2) === "](";
    output += alreadyLinkText ? match : inlineCodeWithGithubFileLinks(task, rawPath, knownFiles);
    cursor = closingEnd;
  }
  return output;
}
