...
test("publication body links structured review file lists and findings", () => {
  const body = publicationCommentBody(
    {
      ...
      review_evidence: {
        head_sha: "feedface",
        base_sha: "baseface",
        changed_files: ["src/app.ts", "src/deleted.ts"],
    ...
    },
    {
      ...
      review: {
        mode: "review",
        evidence_status: "complete",
        reviewed_files: ["src/app.ts", "src/deleted.ts"],
  ...
  assert.match(body, /\[`src\/deleted\.ts`\]\(https:\/\/github\.com\/OpenCoven\/coven-github-webhook\/blob\/baseface\/src\/deleted\.ts\)/);
